...
// ============================================================================
// NEW: Visibility filter — ray-capsule intersection
// ============================================================================

// Tests whether a ray (origin + unit direction) intersects a capsule
// (line segment A->B with given radius). Returns true if occluded.
bool rayCapsuleIntersects(
    const Eigen::Vector3d& ray_origin,
    const Eigen::Vector3d& ray_dir,    // must be unit length
    const Eigen::Vector3d& cap_a,
    const Eigen::Vector3d& cap_b,
    double radius)
{
    const Eigen::Vector3d AB = cap_b - cap_a;
    const Eigen::Vector3d AO = ray_origin - cap_a;

    const double ab_len2  = AB.dot(AB);
    const double ab_dot_d = AB.dot(ray_dir);
    const double ab_dot_o = AB.dot(AO);
    const double d_dot_o  = ray_dir.dot(AO);

    const double denom = ab_len2 - ab_dot_d * ab_dot_d;

    double t, s;
    if (std::abs(denom) < 1e-8) {
        // Ray is parallel to capsule axis — use axis projection
        t = d_dot_o;
        s = ab_dot_o / ab_len2;
    } else {
        t = (ab_len2 * d_dot_o - ab_dot_o * ab_dot_d) / denom;
        s = (ab_dot_d * t - ab_dot_o) / ab_len2;
    }

    if (t < 0.0) return false;  // Intersection point is behind the camera
    s = std::max(0.0, std::min(1.0, s));  // Clamp to capsule segment

    const Eigen::Vector3d closest_on_capsule = cap_a + s * AB;
    const Eigen::Vector3d closest_on_ray     = ray_origin + t * ray_dir;

    return (closest_on_ray - closest_on_capsule).squaredNorm() <= radius * radius;
}

// Returns true if the camera has an unobstructed line of sight to the EE.
// Each occluder pair defines one capsule segment using FK link frame origins.
// NOTE: Verify link names below match your robot's URDF.
//       Run `ros2 run tf2_tools view_frames` and check the tree if unsure.
bool isEEVisible(
    const moveit::core::RobotState& state,
    const Eigen::Vector3d& camera_pos,
    const Eigen::Vector3d& ee_pos,
    const std::vector<std::pair<std::string, std::string>>& occluder_link_pairs,
    double capsule_radius = 0.08)   // 8 cm — tune if needed
{
    const Eigen::Vector3d ray_dir = (ee_pos - camera_pos).normalized();

    for (const auto& [link_a, link_b] : occluder_link_pairs) {
        const Eigen::Vector3d cap_a = state.getGlobalLinkTransform(link_a).translation();
        const Eigen::Vector3d cap_b = state.getGlobalLinkTransform(link_b).translation();

        if (rayCapsuleIntersects(camera_pos, ray_dir, cap_a, cap_b, capsule_radius)) {
            return false;  // This link is blocking the view
        }
    }
    return true;
}
...
// CHANGED: now accepts camera_pos + occluder_link_pairs to filter occluded configs.
    ...
    int num_seeds = 8)
{
    ...
    for (int s = 0; s < num_seeds; s++) {
        ...
        if (seed_state.setFromIK(jmg, target_pose, 0.1)) {
            seed_state.update();  // Ensure FK is up-to-date for visibility check

            // NEW: Only accept this IK solution if the camera can see the EE
            const Eigen::Vector3d ee_pos =
                seed_state.getGlobalLinkTransform(jmg->getLinkModelNames().back()).translation();

            if (!isEEVisible(seed_state, camera_pos, ee_pos, occluder_link_pairs)) {
                continue;  // Arm blocks camera view — discard this config
            }
...
int main(int argc, char * argv[])
{
    ...
    if (current_state) {
        ...
        // ---------------------------------------------------------------
        // NEW: Camera position + occluder link pairs for visibility filter
        //
        // camera_pos: origin of link_head_2 in the robot model's world frame.
        //   This is consistent with getGlobalLinkTransform used inside isEEVisible.
        //
        // occluder_link_pairs: consecutive link-frame origins define one capsule
        //   each. These cover upper arm → wrist. The EE and shoulder base are
        //   excluded — only the links that can realistically swing in front of
        //   the camera matter.
        //
        // ⚠️  If link names don't match your URDF, run:
        //       ros2 run tf2_tools view_frames
        //     and replace names below accordingly.
        // ---------------------------------------------------------------
        const Eigen::Vector3d camera_pos =
            current_state->getGlobalLinkTransform("link_head_2").translation();

        const std::vector<std::pair<std::string, std::string>> occluder_link_pairs = {
            {"link_torso_5",      "link_right_arm_0"},
            {"link_right_arm_0",  "link_right_arm_1"},
            {"link_right_arm_1",  "link_right_arm_2"},
            {"link_right_arm_2",  "link_right_arm_3"},
            {"link_right_arm_3",  "link_right_arm_4"},
            {"link_right_arm_4",  "link_right_arm_5"},
        };
        

        std::cout << "Camera origin (link_head_2): ["
                  << camera_pos.x() << ", "
                  << camera_pos.y() << ", "
        ...
        for (int i = 0; i < (int)sphere_points.size(); i++) {
            ...
            // Try multiple random IK seeds — visibility filter applied inside
            auto configs = sampleIKConfigs(
                current_state, jmg, target_pose, i,
                camera_pos, occluder_link_pairs,   // NEW: passed through
                NUM_IK_SEEDS);

            if (!configs.empty()) {
                orientations_with_ik++;
                std::cout << "  Orientation " << i << ": "
                          << configs.size() << " IK solutions (visibility-filtered)" << std::endl;
            } else {
                std::cout << "  Orientation " << i
                          << ": no unoccluded IK solutions found" << std::endl;
            }
